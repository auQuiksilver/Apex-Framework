...
private [
	...
	'_grid_ao_data','_grid_ao_nearRadius','_grid_region_completionThreshold','_grid_markerEvalTimeout','_estimatedTimeLeft','_true','_false','_endl'
];
waitUntil {(time > 0)};
...
for '_x' from 0 to 1 step 0 do {
	...
	if (_QS_diagTickTimeNow > _fpsCheckDelay) then {
		...
		diag_log format [
			'%1********** SERVER REPORT (TOP) ********** %1FPS: %2 * %1Frame: %3 * %1Time: %4 * %1Player count: %5 * %1Active Scripts: %6 * %1Active SQF Scripts: %7 * %1Active SQS Scripts: %8 * %1Active FSM Scripts: %9 * %1Active Zeus: %10 * %1Created Entities: %11 * %1Deleted Entities: %12 * %1Killed Entities: %13 * %1Respawned Entities: %14 * %1Unit Count: %15 * %1Mission Object count: %16 * %1Entities count: %17 * %1Simple Objects count: %18 *%1********** SERVER REPORT (BOTTOM) **********',
			_endl,
			_fps,
			diag_frameNo,
			(round _timeNow),
