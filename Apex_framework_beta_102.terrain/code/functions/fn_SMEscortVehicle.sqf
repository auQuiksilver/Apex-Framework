...
private [
	...
	'_armoredVehicleDir','_groupSide','_isDedicated','_fuctionNotification','_functionAttack','_spawnPos','_player',
	...
	'_convoyVehicleType','_landMine2','_launch','_nearRoadsPositions','_grpLeader','_worldName'
];
...
_roadsInFront = (_vehiclePosInFront nearRoads 25) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))};
_roadsToDestination = (_vehiclePosToDest nearRoads 25) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))};
_timeOffRoad = 0;
_dmgCount = 0;
_count = 0;
...
_QS_manage_convoy_armorTypes = ["b_apc_tracked_01_aa_f","b_apc_wheeled_01_cannon_f","b_apc_tracked_01_crv_f","b_apc_tracked_01_rcws_f","b_mbt_01_arty_f","b_mbt_01_mlrs_f","b_mbt_01_cannon_f","b_mbt_01_tusk_f","b_t_apc_tracked_01_aa_f","b_t_apc_wheeled_01_cannon_f","b_t_apc_tracked_01_crv_f","b_t_apc_tracked_01_rcws_f","b_t_mbt_01_arty_f","b_t_mbt_01_mlrs_f","b_t_mbt_01_cannon_f","b_t_mbt_01_tusk_f","o_apc_tracked_02_aa_f","o_apc_tracked_02_cannon_f","o_apc_wheeled_02_rcws_f","o_mbt_02_arty_f","o_mbt_02_cannon_f","o_t_apc_tracked_02_aa_ghex_f","o_t_apc_tracked_02_cannon_ghex_f","o_t_apc_wheeled_02_rcws_ghex_f","o_t_mbt_02_arty_ghex_f","o_t_mbt_02_cannon_ghex_f","i_apc_wheeled_03_cannon_f","i_apc_tracked_03_cannon_f","i_mbt_03_cannon_f"];
...
_nearbySpawnPos = {
	...
	for '_x' from 0 to 49 step 1 do {
		...
		if (((([_position select 0,_position select 1] nearRoads 15) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))}) isEqualTo []) && ((nearestTerrainObjects [_position2,_bto,2]) isEqualTo [])) exitWith {};
	};
...
if (_isDedicated) then {
	...
	['NewSideMission',['Escort vehicle (convoy)']] remoteExec ['QS_fnc_showNotification',-2,FALSE];
};
'QS_marker_sideMarker' setMarkerText (format ['%1Escort vehicle (convoy)',(toString [32,32,32])]);
waitUntil {
	sleep 3;
	(!alive _vehicle) ||
	(isNull _vehicle) ||
	(((getPosATL _vehicle) distance _startPosition) > _safezone_radius)
};
...
for '_x' from 0 to 1 step 0 do {
		...
		if (_vehicleAlive) then {
			if (_vehicleCanMove) then {
				if (!(_vehicleIsOnRoad)) then {
					if ( (([_vehiclePos select 0,_vehiclePos select 1] nearRoads 25) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))}) isEqualTo []) then {
						if ((_vehiclePos distance (getPosATL _vehicle)) > 5) then {
							if ((_vehiclePos distance _startPosition) > _safezone_radius) then {
	...
	if (!(_ambushInProgress)) then {
		...
		_roadsInFront = ([_vehiclePosInFront select 0,_vehiclePosInFront select 1] nearRoads 200) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))};
		if (!(_roadsInFront isEqualTo [])) then {
			...
			if (!(_isDedicated)) then {
				_marker3 setMarkerPosLocal (getPosATL _randomRoadSegment);
			};
		};
		_roadsToDestination = ([_vehiclePosToDest select 0,_vehiclePosToDest select 1] nearRoads 200) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))};
		if (!(_roadsToDestination isEqualTo [])) then {
			...
			if (!(_isDedicated)) then {
				_marker4 setMarkerPosLocal (getPosATL _randomRoadSegment);
			};
		};
		...
		if (!(_isDedicated)) then {
			_marker5 setMarkerPosLocal _ambushPos;
		};
		...
		if (_ambushCount >= 3) then {
			...
			if ((_vehSpawnPos distance _base) > 1500) then {
				_nearRoadsPositions = ((_vehSpawnPos nearRoads 300) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))}) apply {(getPosATL _x)};
				...
				_veh = createVehicle ['O_G_Offroad_01_armed_F',_vehSpawnPos,[],0,'NONE'];
				missionNamespace setVariable [
					'QS_analytics_entities_created',
					((missionNamespace getVariable 'QS_analytics_entities_created') + 1),
					FALSE
		...
		if (!(_isDedicated)) then {
			_marker1 setMarkerPosLocal _vehiclePosInFront;
			_marker2 setMarkerPosLocal _vehiclePosToDest;
		};
		comment 'Configure force protection';
	} else {
		comment 'AMBUSH IN PROGRESS, CHECK STATE';
		if (!(_enemyArray isEqualTo [])) then {
			if (({(alive _x)} count _enemyArray) < 4) then {
				for '_x' from 0 to 49 step 1 do {
				...
				if ((_spawnPos distance _vehiclePos) < 1000) then {
					if (_intensity > 2) then {
						_enemyGroupType = selectRandom _enemyGroupTypes_big;
					} else {
						_enemyGroupType = selectRandom _enemyGroupTypes;
		...
		if ((_vehiclePos distance _startPosition) > 600) then {
				...
				if (!(_QS_manage_convoy_armor isEqualTo [])) then {
					{
						...
						if (isNil {_armoredVehicle getVariable _QS_manage_convoy_var}) then {
							...
							if (((_armoredVehicle getVariable _QS_manage_convoy_var) select 0) >= 3) then {
								...
								if ((((_armoredVehicle getVariable _QS_manage_convoy_var) select 1) distance (getPosATL _armoredVehicle)) > ((_armoredVehicle getVariable _QS_manage_convoy_var) select 2)) then {
					...
					if (alive _player) then {
						if (!(_player getUnitTrait 'QS_trait_pilot')) then {
							if (!(captive _player)) then {
								if ((_player distance _vehicle) < 150) then {
									if (({(!isPlayer _x)} count (units (group _player))) isEqualTo 0) then {
	...
	comment "if (((_vehicle distance _destination) < 75) || {(missionNamespace getVariable 'QS_smSuccess')}) exitWith {";
	if ((_vehicle distance _destination) < 75) exitWith {
		comment 'Mission success';
		['TaskSucceeded',['','Truck mission complete!']] remoteExec [_fuctionNotification,-2,FALSE];
		sleep 5;
