...
for '_x' from 0 to 1 step 0 do {
			...
			if (isNull _attachedTo) then {
				if (_lifeState isEqualTo 'INCAPACITATED') then {
					if ([0,_unit] call (missionNamespace getVariable 'QS_fnc_isNearFieldHospital')) then {
						...
						[34,['TaskSucceeded',['',(format ['%1 medevac!',profileName])]]] remoteExec ['QS_fnc_remoteExec',-2,FALSE];
					};
				};
			};
		};
	...
	if (!(_49Opened)) then {
		...
		if (!isNull _d49) then {
			...
			if (_clientFOBRespawnEnabled) then {
				if (missionNamespace getVariable 'QS_module_fob_respawnEnabled') then {
					if ((missionNamespace getVariable 'QS_module_fob_respawnTickets') > 0) then {
						if (_unit getVariable 'QS_revive_waitButton') then {
			...
			_buttonRespawnFOB ctrlCommit 0;
