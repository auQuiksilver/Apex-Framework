...
if (!isNil {_cursorTarget getVariable 'QS_object_GT_0'}) exitWith {
	...
	['sideChat',[WEST,'BLU'],(format ['%1 secured the tracking device in Georgetown (check map)!',(groupID (group player))])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
...
if (!isNil {_cursorTarget getVariable 'QS_object_GT_1'}) exitWith {
	...
	['systemChat',(format ['%1 secured an intel tablet in Georgetown',(groupID (group player))])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
	[71,_cursorTarget,TRUE] remoteExecCall ['QS_fnc_remoteExec',2,FALSE];
};
