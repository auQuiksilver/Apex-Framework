...
private _QS_unitCap = [120,110] select (_worldName isEqualTo 'Tanoa');
private _array = [];
...
private _QS_module_virtualSectors = TRUE;	/*/only if AO type is SC/*/
...
private _QS_module_classic = TRUE;
...
private _QS_module_classic_infReinforce_delay = 30;
private _QS_module_classic_infReinforce_checkDelay = _QS_uiTime + _QS_module_classic_infReinforce_delay;
private _QS_module_classic_infReinforce_playerThreshold = 0; /*/debug 20/*/
...
private _QS_module_classic_vehReinforce_playerThreshold = 0;	/*/debug 20/*/
private _QS_module_classic_vehReinforce_cap = 0;
private _QS_module_classic_vehReinforce_cap_0 = 2;	/*/debug 0/*/
private _QS_module_classic_vehReinforce_cap_1 = 2;	/*/debug 1/*/
private _QS_module_classic_vehReinforce_cap_2 = 2;	/*/debug 1/*/
private _QS_module_classic_vehReinforce_cap_3 = 2;	/*/debug 2/*/
private _QS_module_classic_vehReinforce_cap_4 = 2;	/*/debug 2/*/
...
for '_x' from 0 to 1 step 0 do {
	...
	if (_QS_uiTime > _QS_updateGeneralInfoCheckDelay) then {
		...
		_QS_allAgents = (agents apply {(agent _x)}) select {((!isNull _x) && (local _x))};
	...
	if (_QS_module_grid) then {
		if (_QS_uiTime > _QS_module_grid_checkDelay) then {
			if (missionNamespace getVariable ['QS_grid_AI_triggerInit',FALSE]) then {
				...
				_QS_module_grid_terrainData = [0,_QS_module_grid_aoPos,_QS_module_grid_aoSize,_QS_module_grid_aoData] call (missionNamespace getVariable 'QS_fnc_aoGetTerrainData');
				...
				if (!isNil {_QS_module_grid_terrainData select 4}) then {
					...
					if (_QS_module_civilian_count > 0) then {
						missionNamespace setVariable [
							...
							([_QS_module_grid_aoPos,_QS_module_grid_aoSize,'FOOT',_QS_module_civilian_count,FALSE,TRUE] call (missionNamespace getVariable 'QS_fnc_spawnAmbientCivilians')),
							FALSE
						];
					};
				};
				for '_x' from 0 to 2 step 1 do {
					_QS_module_animalSpawnPosition = ['RADIUS',_QS_module_grid_aoPos,(_QS_module_grid_aoSize * 1.25),'LAND',[],FALSE,[],[],FALSE] call (missionNamespace getVariable 'QS_fnc_findRandomPos');
					if ((_QS_module_animalSpawnPosition distance2D _QS_module_grid_aoPos) < (_QS_module_grid_aoSize * 1.5)) then {
						[_QS_module_animalSpawnPosition,(selectRandomWeighted ['SHEEP',0.3,'GOAT',0.2,'HEN',0.1]),(round (3 + (random 3)))] call (missionNamespace getVariable 'QS_fnc_aoAnimals');
					};
				};
				...
				if (({(!isNull _x)} count (missionNamespace getVariable ['QS_grid_enemyRespawnObjects',[]])) > 0) then {
					if (_QS_uiTime > _QS_module_grid_bldgPatrol_checkDelay) then {
						...
						if ((count _QS_module_grid_bldgPatrolUnits) < _QS_module_grid_bldgPatrolRespawnThreshold) then {
							_QS_module_grid_spawnArray = [0,_QS_module_grid_teamSize,_QS_module_grid_aoPos,_QS_module_grid_aoSize,_QS_module_grid_igPos,_QS_module_grid_aoData,_QS_module_grid_terrainData] call _fn_gridSpawnPatrol;
					...
					if (_QS_uiTime > _QS_module_grid_areaPatrol_checkDelay) then {
						...
						if ((count _QS_module_grid_areaPatrolUnits) < _QS_module_grid_areaPatrolRespawnThreshold) then {
							_QS_module_grid_spawnArray = [1,_QS_module_grid_teamSize,_QS_module_grid_aoPos,_QS_module_grid_aoSize,_QS_module_grid_igPos,_QS_module_grid_aoData,_QS_module_grid_terrainData] call _fn_gridSpawnPatrol;
							if (!(_QS_module_grid_spawnArray isEqualTo [])) then {
								{
