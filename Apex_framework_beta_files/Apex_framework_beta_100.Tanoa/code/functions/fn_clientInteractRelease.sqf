...
if (!((attachedObjects player) isEqualTo [])) then {
	{
		if (!isNull _x) then {
			if (_x isKindOf 'Man') then {
				...
				if ((_unit distance (markerPos 'QS_marker_medevac_hq')) < 4) then {
				...
				if (!isNil {_unit getVariable 'QS_RD_escorted'}) then {
					if (_unit getVariable 'QS_RD_escorted') then {
						...
						if (!(_unit isEqualTo (missionNamespace getVariable 'QS_sideMission_POW'))) then {
							if ((_cursorTarget isEqualTo (missionNamespace getVariable 'QS_prisonBooth')) || {((player distance (markerPos 'QS_marker_gitmo')) < 20)}) then {
