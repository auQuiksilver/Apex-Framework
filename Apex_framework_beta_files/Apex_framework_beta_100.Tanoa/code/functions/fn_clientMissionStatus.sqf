	...
	if ((_playerPos distance2D _position) < _radiusControl) then {
		if ((_playerPos distance2D _position) <= _radiusCapture) then {
			...
			if (!(missionNamespace getVariable 'QS_client_vs_msgEnteredCaptureArea')) then {
				...
				(missionNamespace getVariable 'QS_managed_hints') pushBack [1,FALSE,7.5,-1,'Entering CAPTURE area',[],(serverTime + 15)];
			};
			...
			if (!(missionNamespace getVariable 'QS_client_vs_msgEnteredControlArea')) then {
				...
				(missionNamespace getVariable 'QS_managed_hints') pushBack [1,FALSE,7.5,-1,'Entering CONTROL area',[],(serverTime + 15)];
			};
			if (missionNamespace getVariable 'QS_client_vs_msgEnteredCaptureArea') then {
				missionNamespace setVariable ['QS_client_vs_msgEnteredCaptureArea',FALSE,FALSE];
				(missionNamespace getVariable 'QS_managed_hints') pushBack [1,FALSE,7.5,-1,'Leaving CAPTURE area, entering CONTROL area',[],(serverTime + 15)];
			};
		};
	} else {
		if (missionNamespace getVariable 'QS_client_vs_isInControlArea') then {
		...
		if (missionNamespace getVariable 'QS_client_vs_msgEnteredCaptureArea') then {
			...
			if ((_playerPos distance2D _position) < (_radiusControl * 1.5)) then {
				(missionNamespace getVariable 'QS_managed_hints') pushBack [1,FALSE,7.5,-1,'Leaving CAPTURE area',[],(serverTime + 15)];
			};
		};
		if (missionNamespace getVariable 'QS_client_vs_msgEnteredControlArea') then {
			missionNamespace setVariable ['QS_client_vs_msgEnteredControlArea',FALSE,FALSE];
			if ((_playerPos distance2D _position) < (_radiusControl * 1.5)) then {
				(missionNamespace getVariable 'QS_managed_hints') pushBack [1,FALSE,7.5,-1,'Leaving CONTROL area',[],(serverTime + 15)];
			};
		};
	};
};
...
_QS_flag_west = 'a3\data_f\flags\flag_us_co.paa';
...
_productVersionCtrl ctrlSetTextColor [1,1,1,0.1];
_productVersionCtrl ctrlSetFont 'TahomaB';
_productVersionCtrl ctrlSetText ((missionNamespace getVariable ['QS_system_devBuild_text','Apex Framework (Beta)']) + ' by Quiksilver');
		...
		if ((!(_isStreamFriendly)) && (missionNamespace getVariable ['QS_missionStatus_shown',FALSE])) then {
			if (_currentTaskTimer) then {
				...
				if (_currentTaskTime >= 60) then {
					_currentTaskTimerTextFormat = parseText (format ['%1',([_currentTaskTime,'MM:SS'] call _fn_secondsToString)]);
				} else {
					if (_currentTaskTime >= 20) then {
						_currentTaskTimerTextFormat = parseText (format ['%1',([_currentTaskTime,'MM:SS'] call _fn_secondsToString)]);
					} else {
						if (_currentTaskTime > 0) then {
							_currentTaskTimerTextFormat = parseText (format ['<t color="#ff0000">%1</t>',([_currentTaskTime,'MM:SS'] call _fn_secondsToString)]);
						} else {
							_currentTaskTimerTextFormat = parseText (format ['<t color="#ff0000">%1</t>',([(0 - 0),'MM:SS'] call _fn_secondsToString)]);
						};
					};
				};
