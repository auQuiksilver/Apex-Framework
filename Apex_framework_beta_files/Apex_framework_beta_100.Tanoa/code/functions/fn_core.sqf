...
private [
	...
	'_grid_ao_data','_grid_ao_nearRadius','_grid_region_completionThreshold','_grid_markerEvalTimeout','_estimatedTimeLeft'
];
waitUntil {(time > 0)};
_QS_productVersion = productVersion;
_QS_worldName = worldName;
_QS_worldSize = worldSize;
_unitCap = [120,110] select (_QS_worldName isEqualTo 'Tanoa');
_aoThreshold = 10;
_casThreshold = 12;
_pause = 5;
_timeNow = time;
...
_currentSideMission = '';
/*/
	Old side missions (disabled but still 100% functional):
		QS_fnc_SMHQcoast
		QS_fnc_SMHQfia
		QS_fnc_SMHQind
		QS_fnc_SMHQresearch
...
_QS_module_customMissions = (random 1) > 0.333;
...
_QS_heliDroneRespawnDelay = 3300; /*/2400/*/
...
for '_x' from 0 to 1 step 0 do {
	...
	if (_QS_diagTickTimeNow > _fpsCheckDelay) then {
		...
		diag_log format [
			'* SERVER REPORT * FPS: %1 * Fr: %2 * Ti: %3 * Pc: %4 * AS: %5 * SQF: %6 * SQS: %7 * FSM: %8 * Z: %9 * Cre: %10 * Del: %11 * Kil: %12 * Rsp: %13 * AU: %14 * AMO: %15 * EN: %16 * ASO: %17 *',
			_fps,
			diag_frameNo,
			(round _timeNow),
			_allPlayersCount,
				...
				if ((missionNamespace getVariable 'QS_mission_aoType') isEqualTo 'CLASSIC') then {
					if (!(_defendAOActive)) then {
						if ((!(missionNamespace getVariable 'QS_aoSuspended')) && (!(missionNamespace getVariable 'QS_customAO_active'))) then {
							...
							if (!isNil {missionNamespace getVariable 'QS_aoHQ'}) then {
												...
												if (!(_grid_availableRegions isEqualTo [])) then {
													...
													[
														[],
														{
															...
															51 cutText [(format ["<img size='5' image='%1'/><br/><br/><t size='3'>%2 Campaign completed</t>",_endImage,worldName]),'PLAIN DOWN',5,TRUE,TRUE];
														}
			...
			if ((!(_QS_module_customMission_played)) || {(missionNamespace getVariable 'QS_customAO_trigger')}) then {
				if (!(_mainMissionActive)) then {
					if (((_timeNow > _QS_module_customMissions_delay) && (_allPlayersCount <= 30) && (_allPlayersCount >= 5)) || {(missionNamespace getVariable 'QS_customAO_trigger')}) then {
						...
						missionNamespace setVariable ['QS_customAO_script',(TRUE spawn (missionNamespace getVariable _QS_module_customMission_selected)),FALSE];
					};
				};
			};
		} else {
								...
								if ((isNull (ropeAttachedTo _v)) && (isNull (isVehicleCargo _v)) && (isNull (attachedTo _v))) then {
									if ((_v distance2D _vpos) > 10) then {
										...
										if (((crew _v) isEqualTo []) || {(({(alive _x)} count (crew _v)) isEqualTo 0)}) then {
											if (([_posCheck,_distReal,[WEST,CIVILIAN],(if (_v isKindOf 'Helicopter') then [{(_allPlayers select {(_x getUnitTrait 'QS_trait_pilot')})},{_allPlayers}]),0] call _fn_serverDetector) isEqualTo []) then {
												if ([_vpos,_nearEntitiesCheck] call _fn_isPosSafe) then {
													missionNamespace setVariable [
														'QS_analytics_entities_deleted',
														((missionNamespace getVariable 'QS_analytics_entities_deleted') + 1),
	...
	if (_timeNow > _QS_cleanup_checkDelay) then {
		...
		if (_timeNow > _eastVehicles_delay) then {
			...
			if (!(_eastVehicles isEqualTo [])) then {
				{
					_x setVehicleAmmo 1;
					if (isNil {_x getVariable 'QS_disableRefuel'}) then {
						_x setFuel 1;
					};
				} count _eastVehicles;
			};
	...
	if (_QS_messagingSystem) then {
		if (_timeNow > _QS_message_delay) then {
			...
			if (_QS_messageCount > 0) then {
				...
				['systemChat',_QS_messageCurrent] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
			};
	...
	if (_QS_module_opsec) then {
		if (_timeNow > _QS_module_opsec_checkDelay) then {
			...
			if (_QS_module_opsec_checkMarkers) then {
				_QS_allMapMarkers = allMapMarkers;
				{
					if (!(_x isEqualTo 'respawn_west')) then {
...
TRUE spawn _QS_module_restart_script;