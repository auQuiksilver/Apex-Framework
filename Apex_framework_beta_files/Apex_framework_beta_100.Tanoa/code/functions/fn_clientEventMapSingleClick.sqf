	...
	if (_shift) then {
		if ((markerAlpha 'QS_marker_GT_TP') isEqualTo 1) then {
			if ((_pos distance2D (markerPos 'QS_marker_GT_TP')) < 100) then {
				if ((player distance2D [5762,10367,0]) > 1000) then {
					if (isNull (objectParent player)) then {
						if (({((!isNull _x) && (!(_x isKindOf 'Sign_Sphere10cm_F')))} count (attachedObjects player)) isEqualTo 0) then {
							if (((lifeState player) in ['HEALTHY','INJURED']) && (!captive player)) then {
								...
								_setPos = (markerPos 'QS_marker_GT_TP') getPos [(random 30),(random 360)];
								0 spawn {
									player allowDamage FALSE;
									uiSleep 10;
									player allowDamage TRUE;
								};
								player setVehiclePosition [(AGLToASL _setPos),[],0,'NONE'];
								openMap FALSE;
