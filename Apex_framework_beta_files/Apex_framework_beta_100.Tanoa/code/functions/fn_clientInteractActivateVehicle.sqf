...
if ((player distance2D _cursorObject) > 49) exitWith {
	50 cutText ['Too far','PLAIN DOWN',0.5];
};
private _exit = FALSE;
...
if (!((uiNamespace getVariable 'QS_vehicle_activations') isEqualTo [])) then {
	uiNamespace setVariable ['QS_vehicle_activations',((uiNamespace getVariable 'QS_vehicle_activations') select {(_x > (_time - 600))})];
	if ((count (uiNamespace getVariable 'QS_vehicle_activations')) >= 3) then {
		_exit = TRUE;
	};
};
if (_exit) exitWith {
	50 cutText ['Too many vehicle activations (3+ within 10 minutes), try again soon','PLAIN DOWN',0.5];
};
