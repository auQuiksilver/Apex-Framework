...
if (_t in _mora) then {
	if (!(_z)) then {
		_u setObjectTextureGlobal [0,'media\images\vskins\fv720\apc_tracked_03_ext_blufor_co.paa'];
		_u setObjectTextureGlobal [1,'media\images\vskins\fv720\apc_tracked_03_ext2_blufor_co.paa'];
	};
};
...
if ((toLower _t) in ['i_plane_fighter_03_cas_f','i_plane_fighter_03_aa_f','i_plane_fighter_03_dynamicloadout_f']) then {
	{
		_u setObjectTextureGlobal _x;
	} forEach [
		[0,'media\images\vskins\a143\Buzzard00.paa'],
		[1,'media\images\vskins\a143\Buzzard11.paa']
	];
};
if (!(_isSimpleObject)) then {
	if (_t in _van) then {
