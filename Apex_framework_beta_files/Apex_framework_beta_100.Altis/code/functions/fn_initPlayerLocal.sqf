...
Last modified:
	...
	14/10/2017 A3 1.76 by Quiksilver
...
if (['whitelist',_roleDescription,FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
	if (['Spectator',_roleDescription,FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
	...
	if (!((toLower _playerClass) in ['b_fighter_pilot_f'])) then {
		if (['Pilot',_roleDescription,FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
	...
	if (['Medic',_roleDescription,FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
	...
	if (['Sniper',_roleDescription,FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
		if ((!(_uid in ((['S3'] call (missionNamespace getVariable 'QS_fnc_whitelist'))))) && (!(serverCommandAvailable '#logout'))) then {
			_exit = TRUE;
			endMission 'QS_RD_end_1';
		};
	};
	if (['Engineer',_roleDescription,FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
		if ((!(_uid in ((['S3'] call (missionNamespace getVariable 'QS_fnc_whitelist'))))) && (!(serverCommandAvailable '#logout'))) then {
			_exit = TRUE;
			endMission 'QS_RD_end_1';
		};
	};
	if (['Commander',_roleDescription,FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
...
if (['disabled',_roleDescription,FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
	_exit = TRUE;
	endMission 'end1';
};
if (_exit) exitWith {};
...
player setUnitTrait ['QS_trait_JTAC',(['jtac',_playerClass,FALSE] call (missionNamespace getVariable 'QS_fnc_inString')),TRUE];
...
if ((toLower _playerClass) in ['b_soldier_uav_f','b_t_soldier_uav_f']) then {
	...
	TRUE spawn (missionNamespace getVariable 'QS_fnc_uavOperator');
};
TRUE spawn {
	uiSleep 3;
...
[] spawn (missionNamespace getVariable 'QS_fnc_clientDiary');
TRUE spawn (missionNamespace getVariable 'QS_fnc_icons');
...
0 spawn (missionNamespace getVariable ['QS_fnc_clientCore',{if (userInputDisabled) then {disableUserInput FALSE;};endMission 'QS_RD_end_2';}]);
