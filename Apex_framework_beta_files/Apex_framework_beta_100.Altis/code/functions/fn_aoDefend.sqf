...
for '_x' from 0 to 1 step 0 do {
	...
	if (_timeNow > _checkHeldInitialDelay) then {
		if (_timeNow > _checkHeldDelay) then {
				...
				if (_enemyInHQCount >= 15) then {
					...
					if (!(_exitFail)) then {
						...
						if ((round((_sectorControlTicker / _sectorControlThreshold) * 100)) >= 100) then {
							...
							['systemChat',(format ['HQ is %1 percent taken!',(round((_sectorControlTicker / _sectorControlThreshold) * 100))])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
						};
					};
				} else {
			...
			[_taskID,TRUE,(0 max (_sectorControlTicker / _sectorControlThreshold) min 1)] call (missionNamespace getVariable 'QS_fnc_taskSetProgress');
			_checkHeldDelay = time + 30;
		};
	};
