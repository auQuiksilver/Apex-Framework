...
private [
	...
	'_QS_action_activateVehicle','_QS_action_activateVehicle_text','_QS_action_activateVehicle_array','_QS_interaction_activateVehicle','_QS_cursorIntersections','_threadSleep',
	...
	'_hintPriority','_hintUseSound','_hintDuration','_hintPreset','_hintText','_hintOtherData','_hintIrrelevantWhen','_hintTextPrevious','_hintPriorClosedAt'
];
...
_QS_action_ears_array = [_QS_action_ears_text,(missionNamespace getVariable 'QS_fnc_clientInteractEar'),[],50,TRUE,TRUE,'','TRUE',5,FALSE,''];
_QS_interaction_ears = FALSE;
/*/===== Teeth collector/*/
_QS_action_teeth = nil;
_QS_action_teeth_text = 'Collect gold tooth';
_QS_action_teeth_array = [_QS_action_teeth_text,(missionNamespace getVariable 'QS_fnc_clientInteractTooth'),[],51,TRUE,TRUE,'','TRUE',5,FALSE,''];
	...
	_QS_liveFeed_action_1 = _QS_liveFeed_display addAction [
		'Turn on live feed',
		{
			if (isPipEnabled) then {
				...
				50 cutText ['Enable PiP to view the live feed','PLAIN DOWN'];
			};
		},
		[],
		90,
...
if (_QS_module_opsec) then {
	...
	_QS_module_opsec_chatIntercept_code = {
		...
		for '_x' from 0 to 1 step 0 do {
			...
			if ([_chatTextLower,0] call (missionNamespace getVariable 'QS_fnc_ahScanString')) then {
		...
		for '_i' from 0 to ((count _binConfigPatches) - 1) step 1 do {
			...
			if (isClass _patchEntry) then {
				if (!((configName _patchEntry) in _patchList)) then {
					if ((!(['jsrs',(configName _patchEntry),FALSE] call (missionNamespace getVariable 'QS_fnc_inString'))) && (!(['jpex',(configName _patchEntry),FALSE] call (missionNamespace getVariable 'QS_fnc_inString')))) then {
						if (!(['blastcore',(configName _patchEntry),FALSE] call (missionNamespace getVariable 'QS_fnc_inString'))) then {
...
/*/_position = missionNamespace getVariable ['QS_spawnPoint_1',[(markerPos 'QS_marker_base_marker')]];/*/
if (_QS_worldName isEqualTo 'Altis') then {
...
if (_QS_worldName isEqualTo 'Stratis') then {};
if (_QS_worldName isEqualTo 'Takistan') then {};
if (_QS_worldName isEqualTo 'Chernarus') then {};
if (_QS_worldName isEqualTo 'Sahrani') then {};
...
[(((player getVariable _QS_viewSettings_var) select 1) select 0),_position,_QS_viewSettings_var] spawn {
	...
	waitUntil {
		uiSleep 0.01;
		(((player distance _position) < 2000) || (diag_tickTime > _timeout))
	};
	uiSleep 0.01;
...
uiSleep 0.25;
TRUE spawn (missionNamespace getVariable 'QS_fnc_clientMissionStatus');
...
for '_x' from 0 to 1 step 0 do {
				...
				if (!(_QS_player getVariable 'QS_RD_interaction_busy')) then {
						...
						if ((_cursorObject isKindOf 'LandVehicle') || {(_cursorObject isKindOf 'Air')} || {(_QS_v2 isKindOf 'LandVehicle')} || {(_QS_v2 isKindOf 'Air')}) then {
							if (!(missionNamespace getVariable 'QS_repairing_vehicle')) then {
								if (((speed _QS_v2) < 1) && ((speed _QS_v2) > -1)) then {
									...
									if ((_nearSite) || ((!((missionNamespace getVariable ['QS_missionConfig_carrierEnabled',0]) isEqualTo 0)) && (['INPOLYGON_FOOT',player] call _fn_carrier))) then {
					...
					if (isNull _objectParent) then {
						if (!isNull _cursorTarget) then {
							if (_cursorDistance < 7.5) then {
								if (_cursorTarget isKindOf 'Man') then {
									if (alive _cursorTarget) then {
										if (!(captive _cursorTarget)) then {
											if ((lifeState _cursorTarget) in ['HEALTHY','INJURED']) then {
												if (isNull (objectParent _cursorTarget)) then {
													if (!isNil {_cursorTarget getVariable 'QS_surrenderable'}) then {
														if (_cursorTarget getVariable 'QS_surrenderable') then {
					...
					if (isNull _objectParent) then {
						if (!isNull _cursorObject) then {
							if (_cursorObjectDistance <= 2) then {				
								if ((_cursorObject isKindOf 'LandVehicle') || {(_cursorObject isKindOf 'Ship')}) then {
									if (simulationEnabled _cursorObject) then {
										if (!((attachedObjects _cursorObject) isEqualTo [])) then {
								...
								if (isSimpleObject _cursorObject) then {
									if (!((typeOf _cursorObject) isEqualTo '')) then {
										if (_cursorObject isKindOf 'AllVehicles') then {
											if ((((_cursorObject distance2D [5762,10367,0]) > 1000) && (_QS_worldName isEqualTo 'Tanoa')) || {(!(_QS_worldName isEqualTo 'Tanoa'))}) then {
					...
					if ((!((missionNamespace getVariable ['QS_missionConfig_carrierEnabled',0]) isEqualTo 0)) && (!isNull (missionNamespace getVariable ['QS_carrierObject',objNull]))) then {
	...
	/*/========== Thermal Batteries Module/*/
	
	/*/call (missionNamespace getVariable 'LARs_fnc_TIBattery');/*/
						...
						if (!isNil {_QS_crewIndicatorUI}) then {
							...
							if (!isNull (objectParent player)) then {
								if (!(_QS_v2TypeL in [
									...
									if (!(({(alive _x)} count (crew _QS_v2)) isEqualTo 0)) then {
										_fullCrew = fullCrew _QS_v2;
										_crewManifest = '';
										_index = 0;
										for '_x' from 0 to ((count _fullCrew) - 1) do {
											_unit = (_fullCrew select _index) select 0;
											_role = toUpper ((_fullCrew select _index) select 1);
											_cargoIndex = (_fullCrew select _index) select 2;
											_turretPath = (_fullCrew select _index) select 3;
											_personTurret = (_fullCrew select _index) select 4;
											_text = '';
											_roleImg = '';
	...
	if (_QS_module_gearManager) then {
		...
		/*/if (_timeNow > _QS_module_gearManager_checkDelay) then {/*/
			if ((count _allPlayers) > _playerThreshold) then {
	...
	if (_QS_module_handleHeal) then {
		if (_timeNow > _QS_module_handleHeal_checkDelay) then {
			if (!(({(!isPlayer _x)} count (units (group player))) isEqualTo 0)) then {
				{
					_unit = _x;
							...
							if (!(isNil '_opsec_actionParams')) then {
								if (!(_opsec_actionParams isEqualTo [])) then {
									...
									if (!(_opsec_actionTitle in _opsec_actionWhitelist)) then {
										if (!(_puid in (['DEVELOPER'] call _fn_uidStaff))) then {
											if ((!(['ROBOCOP',_opsec_actionTitle,FALSE] call _fn_inString)) && (!(['Put',_opsec_actionTitle,FALSE] call _fn_inString))) then {
		...
		if (_QS_module_opsec_chatIntercept) then {
			if (!isNull (findDisplay 24)) then {
				if (scriptDone _QS_module_opsec_chatIntercept_script) then {
					_QS_module_opsec_chatIntercept_script = [(findDisplay 24)] spawn _QS_module_opsec_chatIntercept_code;
				};
			};
		};
	};
	...
	if (_QS_uiTime > _hintCheckDelay) then {
		...
		if (!(_hintActive)) then {
			if (!(_hintsQueue isEqualTo [])) then {
				...
				if ((!((toLower (str _hintText)) isEqualTo (toLower (str _hintTextPrevious)))) || {(_QS_uiTime > (_hintPriorClosedAt + 30))}) then {
