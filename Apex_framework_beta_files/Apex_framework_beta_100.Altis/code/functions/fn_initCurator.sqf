...
Description:
	...
	Initialize curator module
	[0,QS_testCurator,(getPlayerUID QS_testCurator),54] call QS_fnc_initCurator;
	comment 'https://community.bistudio.com/wiki/configSourceAddonList';
	
	missionNamespace setVariable ['QS_testCurator',cursorTarget,TRUE];
...
if (_type isEqualTo 0) then {
	...
	if (!(_puid isEqualTo '76561198084065754')) then {
		if (missionNamespace getVariable ['QS_customAO_GT_active',FALSE]) then {
			_module setCuratorEditingAreaType FALSE;
			_module addCuratorEditingArea [0,[5762,10367,0],600];
			_module allowCuratorLogicIgnoreAreas FALSE;
		};
		/*/
		if (missionNamespace getVariable ['QS_virtualSectors_active',FALSE]) then {
			_module setCuratorEditingAreaType FALSE;
			_module addCuratorEditingArea [1,(missionNamespace getVariable 'QS_AOpos'),((missionNamespace getVariable 'QS_aoSize') * 1.2)];
			_module allowCuratorLogicIgnoreAreas FALSE;
		};
		/*/
	};
