...
comment 'Roads';
_nearRoads = (_centerPos nearRoads _areaOrRadiusInterrupt) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))};
...
private _nearHouses = (nearestObjects [_centerPos,['House','Building'],_areaOrRadiusInterrupt]) select {(!((_x buildingPos -1) isEqualTo []))};
