...
if (_type isEqualTo 2) then {
	(missionNamespace getVariable 'QS_managed_hints') pushBack [5,FALSE,7.5,-1,'ROBOCOP: No incident report has been filed.',[],(serverTime + 15)];
	[41,[0,_uid,_causedBy,_nameCausedBy,player,_val,TRUE]] remoteExec ['QS_fnc_remoteExec',2,FALSE];
};
if (_type isEqualTo 1) then {
	_text = parseText format ['ROBOCOP: An incident report has been filed and in-game staff have been notified. Thank you for your assistance. You are on server:<br/><br/>%2.',serverName];
	(missionNamespace getVariable 'QS_managed_hints') pushBack [5,FALSE,10,-1,_text,[],(serverTime + 20)];
