...
if (_isHauling && (_towedVehicle isKindOf 'StaticWeapon')) exitWith {
	playSound3D [
...
if (_isUAV) then {
	_vehicle setVariable [
		...
		(_vehicle addAction ['Load cargo',(_vehicle getVariable 'QS_fnc_clientInteractTowLoadCargo'),[],20,FALSE,TRUE,'','[cameraOn] call (missionNamespace getVariable "QS_fnc_conditionTowLoadCargo")',15,FALSE]),
		FALSE
	];
	...
	_vehicle setVariable [
		...
		(player addAction ['Load cargo',(_vehicle getVariable 'QS_fnc_clientInteractTowLoadCargo'),[],20,FALSE,TRUE,'','[(vehicle player)] call (missionNamespace getVariable "QS_fnc_conditionTowLoadCargo")',15,FALSE]),
		FALSE
	];
