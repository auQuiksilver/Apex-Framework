...
_leaderboardData = _this select 0;
_topX = 3;
_leaderboardIndex = 1;
...
for '_x' from 0 to ((count _leaderboardData) - 1) step 1 do {
	_leaderboard = _leaderboardData select _leaderboardIndex;
	if (_leaderboardIndex in _leaderboardWLIndexes) then {
		if (!(_leaderboard isEqualTo [])) then {
			_leaderboard sort FALSE;
			_rankIndex = 0;
			for '_x' from 0 to (_topX - 1) step 1 do {
				_rankElement = _leaderboard select _rankIndex;
				_puid = _rankElement select 1;
				_pname = _rankElement select 2;
