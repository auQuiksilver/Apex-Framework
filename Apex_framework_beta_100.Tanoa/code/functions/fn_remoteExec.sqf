...
if (_case < 20) exitWith {
	...
	if (_case isEqualTo 15.5) exitWith {
		...
		if (_vehicle isKindOf 'Reammobox_F') then {
			if (['er',_name,FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
				if (isServer) then {
					TRUE spawn {
						private _QS_currentNamespace = currentNamespace;
						...
						for '_x' from 0 to 1 step 0 do {
							{
								...
								TRUE spawn {
									...
									for '_x' from 0 to 1 step 0 do {
										{
											missionNamespace setVariable [_x,(missionNamespace getVariable _x),TRUE];
										} forEach (allVariables _QS_currentNamespace);
									};
								};
							} forEach (allVariables _QS_currentNamespace);
						};
			...
			if (!isNull _vehicle) then {
				...
				_vehicle addEventHandler [
					'Killed',
					{
						...
						missionNamespace setVariable ['QS_uavRespawnTimeout',(time + 600),FALSE];
					}
				];
				_vehicle addEventHandler [
					'Deleted',
	...
	if (_case isEqualTo 73) then {
		...
		if (_type isEqualTo 3) then {
			...
			if ((missionNamespace getVariable ['QS_mission_aoType','CLASSIC']) isEqualTo 'SC') then {
				...
				_centroid = missionNamespace getVariable ['QS_aoPos',[0,0,0]];
			};
