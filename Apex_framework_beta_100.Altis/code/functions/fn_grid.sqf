...
Notes:
	...
	Pre-compile marker data so we dont have to procedurally generate each init (later date)
	
	profileNamespace getVariable ['QS_grid_data_persistent',[]];
	
	QS_data
	
	['REGION_SETSTATE',1,0] call QS_fnc_grid;
	
	['INIT'] call QS_fnc_grid;
	
	REGION_GETAVAILABLE
	
	['REGION_GETAVAILABLE',1,1] call QS_fnc_grid;
...
if (_type isEqualTo 'INIT') exitWith {
	...
	if (_grid_data_persistent isEqualTo []) then {
		_grid_data_persistent = call (compile (preprocessFileLineNumbers '\annex\cfg\QS_data_grid.sqf')); 
	};
	private _tempData = [];
	private _regionAOs = [];
