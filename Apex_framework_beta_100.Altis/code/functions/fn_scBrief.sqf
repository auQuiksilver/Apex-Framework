	...
	if (_scWinningSide in [0,2]) then {
		...
		[
			[],
			{
				...
				if (!isStreamFriendlyUIEnabled) then {
					_text = parseText format ["<t align='center' size='2'>Mission Success</t><br/><br/><img size='7' image='%1'/><br/><br/>We dominated them soldiers, get ready for the next one!",'a3\data_f\flags\flag_us_co.paa'];
					(missionNamespace getVariable 'QS_managed_hints') pushBack [2,FALSE,7.5,-1,_text,[],-1];
				};
			}
		] remoteExec ['spawn',-2,FALSE];
