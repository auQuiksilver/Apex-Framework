...
if (_type isEqualTo 'GRID_MARKERS') exitWith {
	...
	'QS_marker_grid_capState' setMarkerText (format ['   Grids converted: 0 / %1',_requiredCount]);
	_objectiveCode = {
		...
		if (!( (markerText 'QS_marker_grid_capState') isEqualTo (format ['   Grids converted: %1 / %2',_currentCount,_requiredCount]))) then {
			'QS_marker_grid_capState' setMarkerText (format ['   Grids converted: %1 / %2',_currentCount,_requiredCount]);
		};
...
if (_type isEqualTo 'SITE_TUNNEL') exitWith {
	...
	'QS_marker_grid_rspState' setMarkerText (format ['   Tunnels destroyed: 0 / %1',(missionNamespace getVariable 'QS_grid_AIRspTotal')]);
	if (!(_entities isEqualTo [])) then {
		...
		_objectiveCode = {
			...
			if (!((toLower (markerText 'QS_marker_grid_rspState')) isEqualTo (toLower (format ['   Tunnels destroyed: %1 / %2',((_entitiesTotal - _aliveCount) max 0),(missionNamespace getVariable 'QS_grid_AIRspTotal')])))) then {
				'QS_marker_grid_rspState' setMarkerText (format ['   Tunnels destroyed: %1 / %2',(_entitiesTotal - _aliveCount),(missionNamespace getVariable 'QS_grid_AIRspTotal')]);
			};
			...
			if (!(_uxoArray isEqualTo [])) then {
				...
				_objectiveOnCompleted = {
					['GRID_IDAP_UPDATE',['Area Of Operations','Unexploded Ordnance (UXO) cleared']] remoteExec ['QS_fnc_showNotification',-2,FALSE];
					{
						...
						_x setMarkerText '   UXO Field (cleared)';
					} forEach [
						'QS_marker_grid_IDAPmkr',
						'QS_marker_grid_IDAPcircle'
					];
