...
params [
	'_QS_actionTarget','_QS_player','_QS_actionIndex','_QS_actionName','_QS_actionText','_QS_actionPriority','_QS_actionShownWindow','_QS_actionHiddenOnUse',
	'_QS_actionShortcutName','_QS_actionVisibility','_QS_actionEventName'
];
private _QS_c = FALSE;
...
private _exit = FALSE;
private _text = '';
private _QS_module_opsec = TRUE;			/*/ Set this to FALSE to disable clientside anticheat. /*/
	...
	if (!((_this select 4) isEqualTo '')) then {
		if (_QS_module_opsec) then {
			if (!((getPlayerUID player) in (['DEVELOPER'] call (missionNamespace getVariable 'QS_fnc_whitelist')))) then {
				...
				if ((!((_this select 4) in _whitelistedActions)) && (!(['ROBOCOP',(_this select 4),FALSE] call (missionNamespace getVariable 'QS_fnc_inString')))) then {
					_exit = TRUE;
					[
						40,
						[
	...
	if (!(_QS_c)) then {
		if (304380 in (getDLCs 2)) then {
			if (['pilot',(typeOf player),FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
				if (isText (configFile >> 'CfgVehicles' >> (typeOf (_this select 0)) >> 'DLC')) then {
...
if (_QS_actionName in ['ListRightVehicleDisplay','NextModeRightVehicleDisplay']) then {
	_QS_c = TRUE;
	50 cutText ['Please bind these actions to keys. [Esc]>>[Configure]>>[Controls]>>[Keyboard]>>[Common]>>[Panels]. Suggest [ and ] keys','PLAIN',2];
};
