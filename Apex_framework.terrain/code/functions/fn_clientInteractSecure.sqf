...
if (!isNil {_cursorTarget getVariable 'QS_object_GT_0'}) exitWith {
	...
	['sideChat',[WEST,'BLU'],(format [localize 'STR_QS_Chat_030',(groupID (group player)),(['Kavala','Georgetown'] select (worldName isEqualTo 'Tanoa'))])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
...
if (!isNil {_cursorTarget getVariable 'QS_sc_subObj_1'}) exitWith {
	...
	['sideChat',[WEST,'BLU'],(format [localize 'STR_QS_Chat_033',(groupID (group player))])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
...
if (!isNil {_cursorTarget getVariable 'QS_sc_subObj_3'}) exitWith {
	player playAction 'PutDown';
	50 cutText [localize 'STR_QS_Text_136','PLAIN DOWN',0.75];
	['sideChat',[WEST,'BLU'],(format [localize 'STR_QS_Chat_034',(groupID (group player))])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
...
player playAction 'PutDown';
['sideChat',[WEST,'BLU'],(format [localize 'STR_QS_Chat_035',(groupID (group player))])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
