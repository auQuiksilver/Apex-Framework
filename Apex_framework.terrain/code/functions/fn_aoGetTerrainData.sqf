...
if (_type isEqualTo 0) exitWith {
	...
	_roadSegments = ((_position select [0,2]) nearRoads _radius) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))};
	...
	_nearHouses = (nearestObjects [_position,['House'],_radius,TRUE]) select {(!(([_x,(_x buildingPos -1)] call (missionNamespace getVariable 'QS_fnc_customBuildingPositions')) isEqualTo []))};
...
if (_type isEqualTo 1) exitWith {
	...
	if (!(_nearBuildingPositions isEqualTo [])) then {
		...
		if (!(_buildingPositionsInArea isEqualTo [])) then {
			...
			if (!(_buildingPositionsInAreaNearGround isEqualTo [])) then {
	...
	if (!(_forestPositions isEqualTo [])) then {
