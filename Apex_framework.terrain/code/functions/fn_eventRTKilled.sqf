...
if (!isNull _killer) then {
	if (isPlayer _killer) then {
		...
		if ((random 1) > 0.5) then {
			['sideChat',[WEST,'HQ'],(format [localize 'STR_QS_Chat_008',_name])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
		} else {
			['sideChat',[WEST,'HQ'],(format [localize 'STR_QS_Chat_009',_name])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
		};
