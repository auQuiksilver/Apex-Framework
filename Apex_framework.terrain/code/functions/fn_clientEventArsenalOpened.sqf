...
Last Modified:
	...
	15/12/2017 A3 1.80 by Quiksilver
...
0 spawn {
	...
	waitUntil {
		uiSleep 0.1;
		(time > _t) ||
		(!isNull (uiNamespace getVariable ['BIS_fnc_arsenal_display',displayNull]))
	};
	if (!isNull (uiNamespace getVariable ['BIS_fnc_arsenal_display',displayNull])) then {
		...
		for '_x' from 0 to 1 step 0 do {
			...
			if ((count (allControls (uiNamespace getVariable ['BIS_fnc_arsenal_display',displayNull]))) > 203) exitWith {
				[
					40,
					[
						time,
