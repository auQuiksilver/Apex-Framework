...
for '_x' from 0 to 29 step 1 do {
	...
	if ((((missionNamespace getVariable 'QS_registeredPositions') findIf {((_position distance2D _x) < 75)}) isEqualTo -1) && ((((_position select [0,2]) nearRoads 25) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))}) isEqualTo []) && (!((toLower(surfaceType _position)) in ['#gdtasphalt'])) && (!([_position,30,8] call (missionNamespace getVariable 'QS_fnc_waterInRadius')))) exitWith {};
