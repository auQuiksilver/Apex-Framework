			...
			if (isPlayer _driver) then {
				...
				if ((_vehicle distance2D (markerPos 'QS_marker_base_marker')) < 500) then {
					...
					if (_unit in _loadedAtMission) then {
						_val = 1;
						_driver setVariable ['QS_IA_PP_loadedAtMission',(_loadedAtMission select {((alive _x) && (!(_x isEqualTo _unit)))}),(!isDedicated)];
						...
						if (_unit in _loadedAtBase) then {
							_driver setVariable ['QS_IA_PP_loadedAtBase',(_loadedAtBase select {((alive _x) && (!(_x isEqualTo _unit)))}),(!isDedicated)];
						} else {
							...
							if (_unit in _loadedInField) then {
								...
								_driver setVariable ['QS_IA_PP_loadedInField',(_loadedInField select {((alive _x) && (!(_x isEqualTo _unit)))}),(!isDedicated)];
								0 = (missionNamespace getVariable 'QS_leaderboards_session_queue') pushBack ['TRANSPORT',(getPlayerUID _driver),(name _driver),_val];
							};
						};
					};
