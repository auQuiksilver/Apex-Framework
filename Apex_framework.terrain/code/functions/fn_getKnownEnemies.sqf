	...
	if (missionNamespace getVariable ['QS_client_showKnownEnemies',TRUE]) then {
		...
		if ((lifeState _player) in ['HEALTHY','INJURED']) then {
			_hiddenTypes = [
				...
				"o_uav_02_dynamicloadout_f","o_uav_02_cas_f","i_uav_02_dynamicloadout_f","i_uav_02_cas_f"
			];
						...
						if (_target isKindOf 'Man') then {
							...
							if ((!(_targetType in _hiddenTypes)) || {(_showStealth)}) then {
								if (_knownByGroup) then {
									...
									_array pushBack ['a3\ui_f\data\igui\RscCustomInfo\Sensors\Targets\EnemyMan_ca.paa',_color,_targetPosition,_is,_is,0,'',0,0.03,'RobotoCondensed','right'];
								};
							};
						} else {
